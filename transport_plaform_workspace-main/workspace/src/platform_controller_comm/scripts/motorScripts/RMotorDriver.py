...
from pythonLibs.bpnp import PackagePacker, PackageFinder, DataPacker, SENDERS_BY_ID, BPB_ID


TEST_STRING = (b'\x10\x02\xb2\x9f\x10\x03'
               b'\x10\xb3\xb1\x7b\x00\x00\x10\x10\x41\x00\x00\x00\x00\xe0\x10\x03')

TARGET_ID = BPB_ID


class RMotorCommunication:
    def __init__(self, port=None, baudrate=None):
        rospy.init_node('rear_motor_communication_node')
        self.port = port
        ...
        rospy.wait_for_message('front_motor_extracted_data', PacketContent, timeout=None)
        if port is not None:
            if not self._open_port_():
                self.ser = None
                rospy.logerr(f"[RearMotorDriver] Port not found!")
        self.pub = rospy.Publisher('rear_motor_extracted_data', PacketContent, queue_size=10)
        self.sub = rospy.Subscriber('rear_motor_command_data', PacketContent, self.cmd_pack_callback, queue_size=10)
    ...
    def cmd_pack_callback(self, data):
        ...
        if self.ser is not None:
            ...
            rospy.loginfo("[RearMotorDriver] Packed data: %s", byte_str)
    ...
    def _open_port_(self):
        ...
        while attempts < len(available_ports):
            ...
            if self._attempt_open_port_():
                ...
                if res:
                    rospy.loginfo(f"[RearMotorDriver] Port {self.port} is open and functioning correctly.")
                    return True
            rospy.loginfo(f"[RearMotorDriver] Port {self.port} failed. Trying next port.")
            port_index = (port_index + 1) % len(available_ports)
            attempts += 1
        return False

    def _attempt_open_port_(self):
        rospy.loginfo(f"[RearMotorDriver] Attempting to open port {self.port}")
        def attempt():
            try:
                ...
                rospy.loginfo(f"[RearMotorDriver] Failed to open port {self.port}: {e}")
                self.success = False
        ...
        if thread.is_alive():
            rospy.loginfo(f"[RearMotorDriver] Timeout occurred while attempting to open port {self.port}")
            return False
        return self.success
    ...
    def _check_port_(self):
        ...
        if thread.is_alive():
            rospy.loginfo(f"[RearMotorDriver] Timeout occurred while attempting to check open port {self.port}")
            return False
        return self.check
...
if __name__ == '__main__':
    communication = RMotorCommunication('/dev/ttyACM2', 115200)
    try:
        communication.run()
